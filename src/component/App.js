...
function App() {
  ...
  useEffect(() => {
    const fetchBestItems = async () => {
      const { list: productList } = await getProductList({ orderBy: "favorite", pageSize: bestItemSize });
  ...
  const handleGetProductList = useCallback(async (orderQuery) => {
    let queryParams = { orderBy: orderQuery, page: currentPage, pageSize: pageSize };
            ...
            <div className="sort">
              ...
              <div className="sort-menu">
                <button className="sort-toggle" onClick={toggleSortMenu}>
                  {pageSize === 4 ? <img src={ic_sort} alt='ic_sort'></img> : 
                  <div className='sort-context'>
                    {order === 'favorite' ? <p>좋아요순</p> : <p>최신순</p>}
                    <img className='arrow' src={ic_arrow_down} alt='ic_arrow_down' />
                  </div>}
                </button>
                {isToggle && (
