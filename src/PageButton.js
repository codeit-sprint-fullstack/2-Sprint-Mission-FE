...
function PageButton({ totalItems, currentPage, onPageChange }) {
  ...
  return (
    <div className="PagiNation">
      {startPage > 1 && (
        <button className="PageButton ArrowButton" onClick={() => handlePageClick(startPage - 1)}><img src={LeftArrow} alt="Left Arrow" /></button>
      )}
      ...
      {endPage < totalPages && (
        <button className="PageButton ArrowButton" onClick={() => handlePageClick(endPage + 1)}><img src={RightArrow} alt="Right Arrow" /></button>
      )}
    </div>
  );
}
