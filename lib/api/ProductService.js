...
// 상품 목록 API
export async function getProductList({ page = 1, pageSize = 10 }) {
...
export async function getProduct(productId) {
  const res = await get(`/products/${productId}`, {}, true);
  return res.data;
}
...
export async function createProductFavorite(productId) {
  const res = await post(`/products/${productId}/favorite`, {}, true);
  return res.data;
}

export async function deleteProductFavorite(productId) {
  const res = await remove(`/products/${productId}/favorite`, true);
  return res.data;
}
