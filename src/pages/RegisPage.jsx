	...
	const handleSubmit = async () => {
		...
		if (validation.name && validation.description && validation.price && validation.images && validation.tags) {
			...
			if (productId || productId === 0) {
				res = await asyncPatchProductWithId(productId, values);
			} else {
				res = await asyncPostProduct(values);
			}
			console.log(res);
			if (res?.message) {
				...
				<section className={styles.section}>
					<form className={styles.form}>
						<div className={styles.heads}>
							<h1>상품 등록하기</h1>
							<button onClick={handleSubmit} type="button" disabled={!(validation.name && validation.description && validation.price && validation.images && validation.tags) || isPending}>{productId || productId === 0 ? '수정' : '등록'}</button>
						</div>
