...
export default function SortOrder({
  initialOrder = SORT_ORDER_RECENT,
  onChange,
}) {
  ...
  return (
    <div className={styles.sortOder} onClick={toggleDropdown}>
      <div className={styles.customDropdownSelected}>
        ...
        <img
          src={isMobile ? sortBtn : dropdownArrowImg}
          alt="Dropdown"
      ...
      {dropdownOpen && (
        <ul className={styles.customDropdownList}>
          <li
            className={styles.itemRecent}
            onClick={() => handleValueChange(SORT_ORDER_RECENT)}
          >
            최신순
          </li>
          {/* <li
            className={styles.itemFavorite}
            onClick={() => handleValueChange(SORT_ORDER_FAVORITE)}
          >
            좋아요순
          </li> */}
        </ul>
